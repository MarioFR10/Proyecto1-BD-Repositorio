...
-- Create Date: 2020
-- Description: Create Benefactor con events
-- =============================================

ALTER PROCEDURE [dbo].[SP_Create_Benefactor]
(
    @AccountId VARCHAR, 
		...
		BEGIN
			IF exists ( SELECT PR.Id FROM [dbo].[Person] PR WHERE PR.ValueDocIden = @inValueDocIden )
				BEGIN
						...
						EXEC insertarBenefactor @inUsuario
...
ALTER PROCEDURE [dbo].[SP_Read_Benefactor]
(
	@inValueId int
)
AS
...
ALTER PROCEDURE [dbo].[SP_Update_Benefactor]
(
	@beneId INT,
	@inPercentaje INT
)
...
ALTER PROCEDURE [dbo].[SP_Delete_Benefactor]
(
	@beneId int
)
AS
