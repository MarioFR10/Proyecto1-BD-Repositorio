...
/*
ALTER PROCEDURE llenado_Person
(
    @x xml
)
AS
BEGIN
	...
	ELSE
		...
		END
END
exec llenado_Person @x
select * from Person
*/


--------------------------------------------Script de insercion de SavingsAccount

/*
ALTER PROCEDURE llenado_SavingsAccount
(
    @x xml
)
AS
BEGIN
	...
	WHILE @lo1 <= @hi1
		...
		END
END
exec llenado_SavingsAccount @x
select * from SavingsAccount
*/



--------------------------------------------Script de insercion de Benefactor

/*
ALTER PROCEDURE llenado_Benefactor
(
    @x xml
)
AS
...
--------------------------------------------Script de insercion de AccountStatement

/*
ALTER PROCEDURE llenado_AccountStatement
(
    @x xml
)
AS
BEGIN
	...
	WHILE @lo3 <= @hi3
		...
		END
END
exec llenado_AccountStatement @x
select * from AccountStatement
*/



--------------------------------------------Script de insercion de User

/*
ALTER PROCEDURE llenado_User
(
    @x xml
)
AS
BEGIN
	...
	FROM @TempUser
	WHILE @lo4<=@hi4
		BEGIN 			SELECT @valorIdUsu = TU.valordocIdent, 				   @nombre = TU.nombreusu, 				   @pass = TU.contra, 				   @admin = TU.isAdmin			FROM @TempUser TU			WHERE Sec = @lo4			SELECT @usuId = PU.Id			FROM [dbo].[Person] PU			WHERE PU.ValueDocIden = @valorIdUsu			INSERT INTO [dbo].[User] (PersonId, 									  UserName, 									  Password, 									  isAdministrator, 									  InsertAt, 									  InsertBy, 									  InsertIn)			VALUES (@usuId, @nombre, @pass, @admin, GETDATE(), 'Script', '192.168.0.5' )			SET @lo4 = @lo4+1		END
END
exec llenado_User @x
select * from [User]
*/
...
--------------------------------------------Script de insercion de UserCanAccess

/*
ALTER PROCEDURE llenado_UserCanAccess
(
    @x xml
)
AS
BEGIN
	...
	WHILE @lo5 <= @hi5
		...
		END
END
exec llenado_UserCanAccess @x
select * from UserCanAccess
*/
